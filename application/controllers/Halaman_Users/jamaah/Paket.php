...
class Paket extends CI_Controller
{
    ...
    function index()
    {
        ...
        $limit = 5;
        $config['base_url'] = base_url() . 'halaman_depan/paket/index/';
        ...
        $x['data'] = $this->m_paket->berita_perpage($offset, $limit);
    ...
    function detail($slugs)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $data = $this->m_paket->get_berita_by_kode($kode);
            $row = $data->row_array();
            $x['id'] = $row['tulisan_id'];
            ...
            $x['show_komentar'] = $this->m_paket->show_komentar_by_tulisan_id($kode);
            $x['category'] = $this->db->get('tbl_kategori');
            $x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
    ...
    function search()
    {
        ...
        $query = $this->m_paket->cari_berita($keyword);
        if ($query->num_rows() > 0) {
            $x['data'] = $query;
            $x['category'] = $this->db->get('tbl_kategori');
            $x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
