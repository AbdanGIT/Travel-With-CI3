...
class Tulisan extends CI_Controller
{
	...
	function index()
	{
		if ($this->session->userdata('akses') == '1') {
			...
			$this->load->view('halaman_users/admin/v_tulisan', $x);
	...
	function add_tulisan()
	{
		...
		$x['page_title'] = 'Admin | Add Produk';
		$this->load->view('halaman_users/admin/layout/header', $x);
		$this->load->view('halaman_users/admin/layout/sidebar');
		$this->load->view('halaman_users/admin/v_add_tulisan');
		$this->load->view('halaman_users/admin/layout/footer');
	}
	function get_edit()
	{
		$kode = $this->uri->segment(100);
		$x['data'] = $this->m_tulisan->get_tulisan_by_kode($kode);
		$x['kat'] = $this->m_kategori->get_all_kategori();
		$x['page_title'] = 'Admin | Edit Produk';
		$this->load->view('halaman_users/admin/layout/header', $x);
		$this->load->view('halaman_users/admin/layout/sidebar', $x);
		$this->load->view('halaman_users/admin/v_edit_tulisan', $x);
		$this->load->view('halaman_users/admin/layout/footer', $x);
	}
	function simpan_tulisan()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				redirect('halaman_users/admin/tulisan');
			} else {
	...
	function update_tulisan()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				echo $this->session->set_flashdata('msg', 'warning');
				redirect('halaman_users/admin/pengguna');
			}
		} else {
			$tulisan_id = $this->input->post('kode');
			$judul = strip_tags($this->input->post('xjudul'));
